...
st.title(" Datasweeper Sterling Integrator By Huzaifa jafar")
...
#file uploader
uploaded_files = st.file_uploader("Upload your file (accepts CSV or Exvel):",type=["csv","xlsx"],accept_multiple_files=True)
...
if uploaded_files:
    for file in uploaded_files:
        ...
        #file detail
        st.write("Preview the hand of the Dataframe")
        ...
        if st.checkbox(f"Clean data for {file.name}"):
            ...
            with col1:
                if st.button(f"Remove duplicates from yhe file : {file.name}"):
            ...
            if st.checkbox(f"Shoe visualization for {file.name}"):
                st.bar_chart(df.select_dtypes(include='number').iloc[:, :2])

                #conversation option
                st.subheader("Conversation Option")
                conversion_type = st.radio(f"Convert{file.name} to:",["CVS" , "Excel"],key=file.name)
                if st.button(f"Converts{file.name}"):
                    buffer = BytesIO()
